...
export function CanvasNodeSplitDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (params: CanvasImageSplitParams) => void }) {
    ...
    useEffect(() => {
        if (!open) return;
        void readImageMeta(dataUrl).then(setImage).catch(() => setImage(null));
    }, [dataUrl, open]);
                ...
                <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_280px]">
                    ...
                    <div className="space-y-5 py-2">
                        ...
                        <div className="rounded-xl border px-4 py-3 text-sm">
                            <div className="flex items-center justify-between"><span className="opacity-60">子节点</span><span className="font-semibold">{total} 个</span></div>
                            <div className="mt-2 flex items-center justify-between"><span className="opacity-60">单块约</span><span className="font-semibold">{pieceSize ? `${pieceSize.width} x ${pieceSize.height}` : "未知"}</span></div>
                        </div>
                        <Button type="primary" size="large" className="w-full" icon={<Grid2x2 className="size-4" />} onClick={() => onConfirm(params)}>生成子节点</Button>
                    </div>
                </div>
            </div>
        </Modal>
...
function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: string | number | null) => void }) {
    return (
        <label className="block space-y-2">
            ...
            <InputNumber min={1} max={maxGridSize} value={value} onChange={onChange} className="w-full" />
        </label>
    );
}
...
function SplitGrid({ rows, columns }: CanvasImageSplitParams) {
    return (
        <div className="pointer-events-none absolute inset-0 grid" style={{ gridTemplateRows: `repeat(${rows}, 1fr)`, gridTemplateColumns: `repeat(${columns}, 1fr)` }}>
            {Array.from({ length: rows * columns }).map((_, index) => <div key={index} className="border border-white/80 bg-white/5" />)}
        </div>
    );
}

function clampGrid(value: string | number) {
    return Math.max(1, Math.min(maxGridSize, Math.floor(Number(value) || 1)));
}
